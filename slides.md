...
<div class="mt-6 text-xs opacity-70">
Это неприятная, но необходимая функция для основной сети: без неё платформа несовместима с законодательством любой юрисдикции.
</div>
...
- Предложение DAO — это отдельная программа управления, будет в дорожной карте Q3 2026.
...
- ✅ **Голосования нотариусов** (STA-95) + **раунды нотариусов** (STA-101)
...
<div class="mt-8 text-xs opacity-60">
  Работает на Solana devnet. Запуск в основной сети — Q2 2026.
</div>
...
- Голосования нотариусов (STA-95) и раунды нотариусов (STA-101) — закрытые тикеты Jira.
...
Как это работает на уровне блокчейна
</div>

<!--
- Честное предупреждение: дальше 20 минут технических деталей.
- Если вы пришли только за инвест-частью — перерыв хорошая идея.
- Для технарей: часть 2 — это код Anchor, модели PDA, Token-2022, «конверт и вскрытие».
- Держать в голове из части 1: 6 ролей, SPV, 10 тысяч фракций, 7 стадий процесса.
...
## <span class="accent">Для технарей</span>

<div class="opacity-60 mt-4">Архитектура → Контракты → Жизненный цикл → Решения → Вне блокчейна</div>
...
# Полная архитектура — шаг 4 · + внешние сервисы
...
flowchart TB
  ...
  classDef old fill:#152040,stroke:#92c73e,color:#ffffff
  classDef new fill:#92c73e,stroke:#0e1830,color:#0e1830,stroke-width:3px
  class Web,API,DB,R,Prog old
  class Kr,Ir,KYC new
```

<!--
- Kratos — идентичность (email, TOTP, OAuth). Привязка кошелька через EIP-191.
- Irys — постоянное хранение документов. Хеш в блокчейне, файл вне блокчейна.
- Провайдеры KYC пишут подтверждения через webhook в PDA программы `identity`.
- Полный стек: не монолит, а координация 4 независимых слоёв.
- Переход: почему именно Solana для слоя в блокчейне?
-->
...
# 11 программ — шаг 5 · + управление (полная схема)
...
flowchart TB
  ...
  subgraph Gov ["Governance & Trading"]
    Com[compliance]
    Mkt[market]
  end
  Router --> Core
  ...
  classDef old fill:#152040,stroke:#92c73e,color:#ffffff
  classDef new fill:#92c73e,stroke:#0e1830,color:#0e1830,stroke-width:3px
  class Router,Identity,AR,Val,Agr,Exe,Apr,Vlt,Auc old
  class Com,Mkt new
```

<div class="mt-4 text-sm opacity-70">
<strong>compliance</strong> — нотариусы, юристы, комиссии · <strong>market</strong> — вторичный рынок фракций
</div>

<!--
- compliance — самая сложная программа: раунды нотариусов, юридические задачи, конфигурация комиссий.
- market — расчёт вторичных сделок со стаканом ордеров вне блокчейна.
- Всего 11 программ плюс маршрутизатор — 12 развёртываний на Solana.
- Каждая задача делегирована в свою программу — единая ответственность.
...
# Фракционное владение — как это работает

```mermaid {scale: 0.5}
flowchart TB
  subgraph Chain ["В блокчейне (Solana)"]
    Asset[Asset PDA<br/>asset_id, owner, status]
    Mint[Эмитент Token-2022<br/>supply=N, decimals=0]
    Vault[Vault PDA<br/>держит невыпущенный запас]
    TA1[TokenAccount<br/>Инвестор №1: 500]
    TA2[TokenAccount<br/>Инвестор №2: 1500]
    TA3[TokenAccount<br/>Собственник: 8000]
    Asset --- Mint
    Mint --> TA1
    Mint --> TA2
    Mint --> TA3
    Vault -.управляет выпуском.-> Mint
  end
  subgraph Real ["Вне блокчейна (реальный мир)"]
    SPV[SPV — ТОО]
    Flat[🏠 Квартира]
    SPV --- Flat
  end
  Asset -.ссылается на хеш SPV.-> SPV
  classDef ok fill:#92c73e,stroke:#0e1830,color:#0e1830
  classDef dim fill:#152040,stroke:#92c73e,color:#ffffff
  class Asset,Mint,Vault,TA1,TA2,TA3 dim
  class SPV,Flat ok
```

<div class="mt-2 text-xs opacity-70">
Эмитент выпускает N фракций (N — параметр при инициализации хранилища). Хранилище держит запас до сбора средств. После успешного сбора — распределяет по токен-аккаунтам инвесторов. SPV держит реальный актив вне блокчейна. Хеш документов SPV записан в Asset PDA.
</div>

<!--
- Самый важный слайд про токенизацию: как блокчейн связан с реальным миром.
- Слева — блокчейн: Asset, Mint, Vault, TokenAccount. Это цифровые записи.
- Справа — реальный мир: SPV (ТОО) владеет настоящей квартирой.
- Связка: Asset в блокчейне хранит хеш документов SPV. Если SPV изменится — будет видно.
- Эмитент — «станок», который печатает фракции для конкретной квартиры. Для каждой квартиры свой станок.
- Хранилище — сейф. Держит все фракции до продажи, потом раздаёт инвесторам.
- TokenAccount — это как банковский счёт, но для фракций. У каждого инвестора свой.
- Переход: как защищены эти передачи — кто проверяет каждую сделку.
-->
...
```mermaid {scale: 0.45}
flowchart LR
  subgraph Commit ["Запечатывание (7 дней)"]
    C1[Оценщик #1<br/>хеш]
    C2[Оценщик #2<br/>хеш]
    CN[...#N]
  end
  subgraph Reveal ["Вскрытие (7 дней)"]
    R1[открыть цену + соль]
    R2[открыть цену + соль]
    RN[...]
  end
  C1 --> R1
  C2 --> R2
  ...
  R2 --> Med
  RN --> Med
  Med --> Out{цена далеко от медианы?}
  Out -->|да| Slash[штраф 20% залога]
  Out -->|нет| Reward[вернуть залог + комиссия]
  Med --> Final[minimum_price]
  classDef ok fill:#92c73e,stroke:#0e1830,color:#0e1830
  classDef bad fill:#fca5a5,stroke:#991b1b
  class Final,Reward ok
  class Slash bad
```

<div class="mt-2 text-xs opacity-70">
Вес голоса = 1 (оценщики равны). Но штрафное удержание делает плохие голоса дорогими.
</div>
...
# Децентрализованные нотариусы (STA-117)

<div class="mt-2 text-xs opacity-60">
<strong>STA-117</strong> — номер спецификации в нашем внутреннем трекере задач (ST = Slice Task). Это 117-я задача проекта. Реализует самоуправляемый пул нотариусов без централизованного администратора.
</div>
...
<!--
- STA-117 — отдельный уровень управления над общим пулом нотариусов.
...
# Архитектурное решение: логика биржи

<div class="mt-4 text-sm">

**Это не хакатонное упрощение.** Это сознательная архитектура «биржи»: бэкенд прячет блокчейн от массового пользователя, сохраняя полную функциональность для продвинутых.
...
**Маршрут «как обычный сайт» (по умолчанию):**
1. Регистрация email + TOTP
2. Бэкенд выдаёт кастодиальный кошелёк
3. Все действия выглядят как обычный веб-сервис
...
**Маршрут «самостоятельное хранение ключей» (для продвинутых):**
1. Подключение Phantom / Solflare
2. Все подписи — в клиентском кошельке
3. Полный контроль над ключами
4. Прямое взаимодействие с контрактами

**Переход между маршрутами:**
- Из веб2 можно экспортировать ключ и перейти на самостоятельное хранение ключей
- Из самостоятельного хранения можно привязать email для восстановления

</div>
</div>

<div class="mt-4 text-xs opacity-70">
Аналог — Binance, Coinbase: массовый пользователь взаимодействует с интерфейсом биржи, продвинутый — подключает свой кошелёк. Мы воспроизводим эту модель для RWA.
</div>
...
- Внешние технические аудиты до запуска на основной сети
...
- Внешний аудит до запуска на основной сети — обязательное условие, не опция. Планируем подключить Trail of Bits или OtterSec.
...
- Это не баги, а открытые вопросы, требующие продуктовых решений до основной сети.
...
**✅ Q1 2026 — Devnet (готово)**
11 программ, полный интерфейс, заглушка KYC, голосования нотариусов, принудительные переводы

**🔨 Q2 2026 — запуск в основной сети**
Реальные KYC-провайдеры, регуляторный пилот в AIFC, первые 10 токенизированных объектов

**📈 Q3 2026 — вторичный рынок**
AMM для фракций, маркетмейкеры, полноценный запуск стакана ордеров

**🌐 Q4 2026 — расширение**
Мобильное приложение, рынок РФ, пилот в ОАЭ, коммерческая недвижимость
