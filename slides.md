...
* SPV = Special Purpose Vehicle, юрлицо-обёртка (в КЗ — SPC, Special Purpose Company), на которое оформляется квартира. Фракции = доли в этом SPV.
</div>
</div>
...
**Со SPV:**
- SPV (в КЗ — SPC) владеет квартирой по документам
...
<div class="mt-6 text-sm opacity-70">
<strong>SPV (Special Purpose Vehicle)</strong> — юрлицо-прослойка, создаваемое специально под один актив. В Казахстане аналог — <strong>SPC (Special Purpose Company)</strong>: упрощённая форма юрлица с ограниченной сферой деятельности, зарегистрированная под конкретную цель в уставе. Юрист регистрирует SPC, оформляет квартиру на неё, публикует хеш устава в блокчейне.
</div>
...
- SPV — это юридическая коробка, в которую «упаковали» квартиру. В Казахстане аналог — SPC (Special Purpose Company).
- SPC — упрощённая форма юрлица с ограниченной сферой деятельности, регистрируется специально под один актив с целью в уставе.
- Аналогия: представьте, что квартиру поместили в сейф. Сейф принадлежит SPC. Кто держит ключи — у того квартира.
- Ключи = токены. У кого больше токенов — у того больше голосов в управлении SPC.
- На каждую квартиру — своя SPC. Никогда не смешиваем: разные квартиры, разные долги, разные проблемы.
- Юрист регистрирует SPC, оформляет документы, кладёт хеш в блокчейн — чтобы все могли проверить.
...
**Ситуация:** юрист по корпоративному праву, специализация — SPC и SPV.
...
- 30 дней — реалистичный срок регистрации SPC в Казахстане.
...
- Казахстан: SPV = SPC (Special Purpose Company). В других юрисдикциях — LLC, Limited, GmbH.
...
<div><span class="accent font-semibold">SPV</span> — Special Purpose Vehicle, юрлицо-прослойка (в КЗ — SPC, Special Purpose Company), на которое оформлена недвижимость.</div>
...
- Почему нельзя «один смарт-контракт для всех стран»: законы разные. В КЗ — SPC, в Дубае — LLC, в РФ — ООО.
...
<div class="mt-4 text-xs opacity-70">
SPV (в КЗ — SPC) юридически владеет квартирой. Asset PDA в блокчейне хранит хеш документов SPV. Если документы изменятся — хеш не сойдётся, это будет видно всем.
</div>
...
- SPV (в КЗ — SPC, Special Purpose Company) юридически владеет квартирой по бумагам.
...
<div class="mt-2 text-xs opacity-70">
Нотариусы проверяют: SPC зарегистрирована, устав соответствует, квартира оформлена на SPC, хеш совпадает. Если нет — штраф юристу, задача возвращается в общий пул.
</div>
...
# Irys — неизменяемое хранилище документов

<div class="mt-4 text-sm grid grid-cols-2 gap-6">
<div>

**Проблема:** где хранить договоры, фотографии, акты SPV? На обычном сервере их можно подменить.

**Решение:** Irys — децентрализованное хранилище поверх Arweave. Аналог S3, но неизменяемый.

</div>
<div>

**Как это работает:**
1. Загрузил документ → получил уникальный идентификатор
2. Идентификатор записывается в блокчейн
3. Подменить документ невозможно — идентификатор не совпадёт

</div>
</div>

<!--
- Irys — это как цифровая капсула времени. Положил документ — он там навсегда.
- Arweave — блокчейн, заточенный под вечное хранение данных.
- Мы не храним файлы в Solana — это дорого. Только хеш (отпечаток).
- Если кто-то подменит документ, хеш не совпадёт с тем, что записан в блокчейне. Это сразу видно.
- Аналогия: как нотариально заверенная копия. Оригинал в архиве, у вас — печать с номером.
- Переход: SPV — мост между блокчейном и реальным миром.
-->

---

# SPV — мост между блокчейном и реальным миром

<div class="mt-4 text-sm">

SPV (Special Purpose Vehicle) — юридическое лицо, на которое оформляется актив. Токен = доля в SPV = реальное право собственности.

</div>

<div class="mt-4 text-sm grid grid-cols-2 gap-4">
<div class="role-card">
<div class="accent font-semibold">Казахстан</div>
<div class="text-xs mt-1">SPC (Special Purpose Company) через AIFC</div>
</div>
<div class="role-card">
<div class="accent font-semibold">ОАЭ</div>
<div class="text-xs mt-1">LLC через VARA / DIFC</div>
</div>
<div class="role-card">
<div class="accent font-semibold">Великобритания</div>
<div class="text-xs mt-1">Limited Company</div>
</div>
<div class="role-card">
<div class="accent font-semibold">Германия</div>
<div class="text-xs mt-1">GmbH</div>
</div>
</div>

<div class="mt-4 text-xs opacity-70">
В каждой стране свой формат юридического лица. Slice может работать с любой юрисдикцией — хеш документа фиксируется в блокчейне одинаково.
</div>

<!--
- SPV — ключевой мост. Без него токен — просто картинка. С ним — документ с юридической силой.
- В каждой стране свой формат. В Казахстане — SPC через AIFC, в Дубае — LLC, в Германии — GmbH.
- Slice не привязан к одной юрисдикции. Механизм универсальный: создаём юрлицо, оформляем актив, хеш — в блокчейн.
- Это и есть главное отличие от «просто токенов»: за каждым токеном стоит реальный юридический документ.
- Переход: что мы планируем разрабатывать дальше.
-->

---

# Что дальше — ключевые модули в разработке

<div class="mt-4 text-sm grid grid-cols-3 gap-4">

<div class="role-card">
<div class="accent font-semibold">📊 Отчётность и дивиденды</div>
<div class="text-xs mt-2">Актив приносит доход (аренда, выручка). Система автоматически распределяет прибыль между владельцами долей.</div>
</div>

<div class="role-card">
<div class="accent font-semibold">🗳️ Совет директоров</div>
<div class="text-xs mt-2">Владельцы голосуют по ключевым решениям: распределение прибыли, ремонт, смена управляющего, продажа — прямо в блокчейне.</div>
</div>

<div class="role-card">
<div class="accent font-semibold">🌐 Интеграция с госреестрами</div>
<div class="text-xs mt-2">Автоматическая верификация через государственные базы данных, аренда через доли, выход на международные рынки.</div>
</div>

</div>

<!--
- Три главных модуля, которые нужны для полноценного продукта.
- Дивиденды: актив приносит аренду, система сама считает и распределяет. Куда направить — на выплаты или на ремонт?
- Совет директоров: владельцы голосуют без собраний и протоколов. Всё в блокчейне: прозрачно, необратимо, с кворумом.
- Госреестры: на старте верификация ручная (нотариусы). В перспективе — автоматическая через API Минюста, ЕГРП.
- Переход: честно о рисках.
-->

---

# Риски — мы их видим и не прячем

<div class="mt-4 text-xs grid grid-cols-2 gap-4">

<div class="role-card">
<div class="accent font-semibold">⚖️ Регуляция</div>
<div class="mt-1">IPO и рынок активов не просто так устроены сложно. Государство — жёсткий регулятор, и это правильно. В каждой юрисдикции свои правила: что можно токенизировать, как защитить покупателя, кто отвечает при споре.</div>
</div>

<div class="role-card">
<div class="accent font-semibold">📜 Судебная практика</div>
<div class="mt-1">SPV работает, но судебная практика по токенизированной собственности пока минимальна. Законодательство формируется прямо сейчас — правила будут меняться.</div>
</div>

<div class="role-card">
<div class="accent font-semibold">📉 Ликвидность</div>
<div class="mt-1">На старте вторичный рынок будет тонким. Пока не набралась критическая масса участников, продать долю быстро будет сложно.</div>
</div>

<div class="role-card">
<div class="accent font-semibold">🔒 Технологический риск</div>
<div class="mt-1">До публичного запуска нужен внешний аудит контрактов. Мы это понимаем и строим систему так, чтобы каждый из этих рисков закрывался по мере роста.</div>
</div>

</div>

<!--
- Честный слайд для жюри и инвесторов. Мы не прячем проблемы.
- Регуляция: когда мы даём мелкому инвестору доступ к крупным активам, мы берём на себя ответственность уровня IPO.
- Судебная практика: SPV работает десятилетиями в традиционных финансах, но в контексте токенизации кейсов пока мало.
- Ликвидность: классическая проблема курицы и яйца. Решается маркетмейкерами и партнёрствами.
- Технический риск: внешний аудит обязателен. Планируем Trail of Bits или OtterSec.
- Переход: ссылки и контакты.
-->
